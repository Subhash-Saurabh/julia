...
function SamplerRangeInt(a::T, diff::U) where {T<:Integer,U<:Unsigned}
    k = diff+one(U)
    SamplerRangeInt{T,U}(a, k, _maxmultiple(k)) # overflow ok
end

uint_sup(::Type{<:BitInteger}) = UInt32
uint_sup(::Type{<:Union{Int64,UInt64}}) = UInt64
uint_sup(::Type{<:Union{Int128,UInt128}}) = UInt128

function SamplerRangeInt(r::AbstractUnitRange{T}) where T<:BitInteger
    isempty(r) && throw(ArgumentError("range must be non-empty"))
    SamplerRangeInt(first(r), (last(r) - first(r)) % uint_sup(T))
end

Sampler(::AbstractRNG, r::AbstractUnitRange{T}, ::Repetition) where {T<:BitInteger} =
    SamplerRangeInt(r)
...
# SamplerOneTo is responsible for providing the right value of k
...
rand(rng::AbstractRNG, sp::SamplerRangeInt{T,U}) where {T<:BitInteger,U} =
